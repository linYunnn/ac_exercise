...
const peopleInfo = JSON.parse(localStorage.getItem('friendList'))
      ...
      <div class="m-3">
        <div class="card">
          ...
          <div class="card-footer text-muted d-flex flex-column p-2 justify-content-center">
            ...
            <button type="button" class="btn btn-outline-danger btn-sm btn-remove-friend" data-id="${item.id}">X</button>
          </div>
        </div>
      </div>
    </div>`
...
function removeFromFriend(id) {
  const friendIndex = peopleInfo.findIndex(person => person.id === id)
  peopleInfo.splice(friendIndex, 1)
  localStorage.setItem('friendList', JSON.stringify(peopleInfo))
  renderPeopleInfo(peopleInfo)
}
